...
const MobileNavbarBrick = ({ openDrawer = false, onClose }: IProp) => {
  const t = useScopedI18n("brick.navbar");
  ...
  return (
    <div
      ref={navbarRef}
      id="drawer-contact"
      className="fixed top-0 left-0 z-40 h-full transition-transform -translate-x-full bg-white w-full md:w-[20rem]"
      tabIndex={-1}
    >
      <div className="flex justify-between items-start px-6 pt-12">
        ...
        <button
          className="inline-flex items-center justify-center p-2 cursor-pointer bg-tertiary"
          onClick={() => (mobileNavbarDrawer as Drawer).hide()}
        >
          <svg
      ...
      <div className="flex flex-col gap-4 text-secondary font-normal px-6 pt-8 min-h-fit h-full overflow-y-auto">
        <div className="border-b border-gray-light px-4 py-2">
          <Link
            onClick={() => (mobileNavbarDrawer as Drawer).hide()}
            href="#home"
            className="block py-2 px-3 hover:bg-gray-100 md:hover:bg-transparent md:hover:text-blue-700 md:p-0"
          >
            {t("landing")}
          </Link>
        </div>
